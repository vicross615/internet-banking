...
export class MtnComponent implements OnInit, AfterViewInit, OnDestroy {
  public Title = 'MTN AirTime & Data';
  activeService: any;
  ...
  set selectedProductDetails(request: number) {
    ...
    this.airtimeService.productDetailsService
      ...
      .subscribe((res: any) => {
        if (request === 0) {
          this.productDetails = res[0]; // MTN
          this.AirtimeForm.controls['productdetails'].patchValue(
            this.productDetails
          );
        } else if (request === 1) {
          for (const i in res) {
            if (res[i].productnetwork === 'MTN') {
