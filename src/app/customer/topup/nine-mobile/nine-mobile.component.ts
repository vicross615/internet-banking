...
@Component({
  ...
  selector: 'gtibank-nine-mobile',
  templateUrl: '../airtime.html',
  styleUrls: ['./nine-mobile.component.scss'],
  animations: [
...
export class NineMobileComponent implements OnInit, OnDestroy, AfterViewInit {
  Title = '9Mobile AirTime & Data';
  activeService: any;
  ...
  ngOnInit() {
    ...
    this.selectedProductDetails = this.requestTypeIs; // Load Airtime By default
  }
  ngAfterViewInit() {
  ...
  set selectedProductDetails(request: number) {
    this.airtimeService.productDetailsForAirtimePurchase = request;
    this.airtimeService.productDetailsService
      .pipe(untilComponentDestroyed(this))
      .subscribe((res: any) => {
        if (request === 0) {
          this.productDetails = res[2]; // 9Mobile
          this.AirtimeForm.controls['productdetails'].patchValue(
            this.productDetails
          );
        } else if (request === 1) {
          for (const i in res) {
            if (res[i].productnetwork === '9MOBILE') {
  ...
  async onSubmit(form) {
    ...
    await this.airtimeService
      .airtimeAndDataPurchase(form.value)
      .pipe(untilComponentDestroyed(this))
      .subscribe(
        (res: any) => {
