...
@Component({
  ...
  selector: 'gtibank-glo',
  templateUrl: '../airtime.html',
  styleUrls: ['./glo.component.scss'],
  animations: [
...
export class GloComponent implements OnInit, OnDestroy, AfterViewInit {
  Title = 'Glo AirTime & Data';
  activeService: any;
  ...
  set selectedProductDetails(request: number) {
    ...
    this.airtimeService.productDetailsService
      ...
      .subscribe((res: any) => {
        if (request === 0) {
          this.productDetails = res[1]; // Glo
          this.AirtimeForm.controls['productdetails'].patchValue(
            this.productDetails
          );
        } else if (request === 1) {
          for (const i in res) {
            if (res[i].productnetwork === 'GLO') {
